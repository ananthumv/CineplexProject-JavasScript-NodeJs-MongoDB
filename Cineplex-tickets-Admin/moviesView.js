...
function render(data){
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        const deleteMovie = document.createElement('a')
        deleteMovie.innerHTML = "Delete"
        deleteMovie.classList.add('button')
        movie.append(deleteMovie)
        ...
        update.addEventListener('click', (e) => {
            ...
            window.location.href ='./updateMovie.html?id='+ movies.dataset.id        
        })

        deleteMovie.addEventListener('click', (e) => {
            controller.deleteMovie(movies.dataset.id)
        })
